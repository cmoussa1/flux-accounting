...
# be rejected and the job will remain pending *instead* of killed by having an
# exception raised on it. Then, after a flux-accounting update is sent to the
# plugin, the job can proceed to be successfully updated and transition to RUN.
...
test_expect_success 'now we can update the bank on the job because plugin has data' '

...
test_expect_success 'add some queues to the DB' '
	flux account add-queue bronze --max-nodes=1 &&
	flux account add-queue silver --max-nodes=2 &&
	flux account add-queue gold --max-nodes=3
'
...
test_expect_success 'check resource counts for association' '
	...
	jq -e ".mf_priority_map[] | select(.userid == 50001) | .banks[0].cur_nodes == 1" <query.json &&
	jq -e ".mf_priority_map[] | select(.userid == 50001) | .banks[0].cur_run_jobs == 1" <query.json &&
	jq -e ".mf_priority_map[] | select(.userid == 50001) | .banks[0].queue_usage[\"bronze\"].cur_nodes == 1" <query.json &&
	jq -e ".mf_priority_map[] | select(.userid == 50001) | .banks[0].queue_usage[\"bronze\"].cur_run_jobs == 1" <query.json
'

test_expect_success 'second job is held due to queue-specific max_nodes limit' '
	...
	flux jobtap query mf_priority.so > query.json &&
	test_debug "jq -S . <query.json" &&
	jq -e ".mf_priority_map[] | select(.userid == 50001) | .banks[0].held_jobs | length == 1" <query.json
'

test_expect_success 'cancel first job; second job is released now that association under max_nodes' '
	flux cancel ${job1} &&
	...
	jq -e ".mf_priority_map[] | select(.userid == 50001) | .banks[0].queue_usage[\"bronze\"].cur_nodes == 1" <query.json &&
	jq -e ".mf_priority_map[] | select(.userid == 50001) | .banks[0].queue_usage[\"bronze\"].cur_run_jobs == 1" <query.json &&
	jq -e ".mf_priority_map[] | select(.userid == 50001) | .banks[0].held_jobs | length == 0" <query.json
'
...
# has a max_nodes limit of 1. A second-submitted job to the bronze queue will
# be held with BOTH a queue-specific max resources dependency AND a
...
test_expect_success 'second job is held due to queue-specific and association-specific max_nodes limit' '
	job2=$(flux python ${SUBMIT_AS} 50001 --queue=bronze -N1 sleep 60) &&
	flux job wait-event -vt 5 \
		--match-context=description="max-resources-queue" \
		${job2} dependency-add &&

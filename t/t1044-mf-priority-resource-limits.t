...
# one node, the job will be held with a "max-resources-user-limit" until one of
...
# Scenario 2:
# The association submits enough jobs to take up the max-nodes limit; even if
# they submit a job that only specifies cores, the job will still have a
# max-resources dependency added to it until a currently running job completes.
test_expect_success 'submit enough jobs to take up max-nodes limit' '
	job1=$(flux python ${SUBMIT_AS} 5001 -N1 sleep 60) &&
	flux job wait-event -vt 10 ${job1} priority &&
	job2=$(flux python ${SUBMIT_AS} 5001 -N1 sleep 60) &&
...
test_expect_success 'trigger max-resources limit for association' '
	job3=$(flux python ${SUBMIT_AS} 5001 -n1 sleep 60) &&
	flux job wait-event -vt 10 \
		--match-context=description="max-resources-user-limit" \
		${job3} dependency-add &&
...
# dependency is added.
test_expect_success 'edit association to have a large max resources limit' '
	flux account edit-user user1 --max-nodes=5 --max-cores=15 &&
	flux account-priority-update -p ${DB_PATH}
'
...
# are freed.
test_expect_success 'decrease the limits of the association' '
	flux account edit-user user1 --max-running-jobs=2 --max-nodes=3 &&
	flux account-priority-update -p ${DB_PATH}
'
...
test_expect_success 'submit enough jobs to take up both limits' '
	...
	flux job wait-event -vt 10 ${job1} priority &&
	job2=$(flux python ${SUBMIT_AS} 5001 -n2 sleep 60) &&
	flux job wait-event -vt 10 ${job2} priority
'

test_expect_success 'ensure both dependencies get added to job' '
	job3=$(flux python ${SUBMIT_AS} 5001 -N2 sleep 60) &&
	flux job wait-event -vt 10 \
		--match-context=description="max-running-jobs-user-limit" \
		${job3} dependency-add &&
	flux job wait-event -vt 10 \
...
test_expect_success 'if resources limit is still hit, job will not be released' '
	...
	test $(flux jobs -no {state} ${job3}) = DEPEND
'

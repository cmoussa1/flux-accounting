...
# through job.new to be accurately associated with each Association object.
test_expect_success 'reload multi-factor priority plugin with DB initialization' '
	flux jobtap remove mf_priority.so &&
	flux jobtap load ${MULTI_FACTOR_PRIORITY} "config=$(flux account init-plugin)"
'
...
test_expect_success 'association has 0 active, running jobs' '
	flux jobtap query mf_priority.so > query.json &&
	test_debug "jq -S . <query.json" &&
	jq -e \
		".mf_priority_map[] |
...
# # In this set of tests, we ensure that jobs held due to flux-accounting
# # dependencies are properly handled after a plugin reload.
# test_expect_success 'edit association to have max running jobs limit of 1' '
...
# test_expect_success 'check running counts of jobs' '
# 	flux jobtap query mf_priority.so > query.json &&
# 	test_debug "jq -S . <query.json" &&
# 	cat query.json | jq &&
# 	jq -e \
...
# test_expect_success 'job2 now meets requirements to run' '
# 	flux job wait-event -t 10 ${job2} alloc
# '

# test_expect_success 'cancel job2' '
# 	flux cancel ${job2} &&
# 	flux job wait-event -t 10 ${job2} clean
# '

...
SMALL_NO_TIE=${EXPECTED_FILES}/small_no_tie.db
SMALL_TIE=${EXPECTED_FILES}/small_tie.db
SMALL_TIE_ALL=${EXPECTED_FILES}/small_tie_all.db
OUT_OF_INSERT_ORDER=${EXPECTED_FILES}/out_of_insert_order.db

test_expect_success 'create hierarchy output from C++ - small_no_tie.db' '
    flux account-shares -p ${SMALL_NO_TIE} > small_no_tie.test
'

test_expect_success 'compare hierarchy outputs - small_no_tie.db' '
    test_cmp ${EXPECTED_FILES}/small_no_tie.expected small_no_tie.test
'

test_expect_success 'create hierarchy output from C++ - small_tie.db' '
    flux account-shares -p ${SMALL_TIE} > small_tie.test
'

test_expect_success 'compare hierarchy outputs - small_tie.db' '
    test_cmp ${EXPECTED_FILES}/small_tie.expected small_tie.test
'

test_expect_success 'create hierarchy output from C++ - small_tie_all.db' '
    flux account-shares -p ${SMALL_TIE_ALL} > small_tie_all.test
'

test_expect_success 'compare hierarchy outputs - small_tie_all.db' '
    test_cmp ${EXPECTED_FILES}/small_tie_all.expected small_tie_all.test
'

test_expect_success 'create parsable hierarchy output from C++ - small_no_tie.db' '
    flux account-shares -P "|" -p ${SMALL_NO_TIE} > small_no_tie_parsable.test
'

test_expect_success 'compare parsable hierarchy outputs - small_no_tie.db' '
    test_cmp ${EXPECTED_FILES}/small_no_tie_parsable.expected small_no_tie_parsable.test
'

test_expect_success 'create parsable hierarchy output from C++ - small_tie.db' '
    flux account-shares -P "|" -p ${SMALL_TIE} > small_tie_parsable.test
'

test_expect_success 'compare parsable hierarchy outputs - small_tie.db' '
    test_cmp ${EXPECTED_FILES}/small_tie_parsable.expected small_tie_parsable.test
'

test_expect_success 'create parsable hierarchy output from C++ - small_tie_all.db' '
    flux account-shares -P "|" -p ${SMALL_TIE_ALL} > small_tie_all_parsable.test
'

test_expect_success 'compare parsable hierarchy outputs - small_tie_all.db' '
    test_cmp ${EXPECTED_FILES}/small_tie_all_parsable.expected small_tie_all_parsable.test
'

test_expect_success 'create custom parsable hierarchy output from C++ - small_tie.db' '
    flux account-shares -P , -p ${SMALL_TIE} > custom_small_tie_parsable.test
'

test_expect_success 'compare custom parsable hierarchy outputs - small_tie_all.db' '
    test_cmp ${EXPECTED_FILES}/custom_small_tie_parsable.expected custom_small_tie_parsable.test
'

test_expect_success 'output help message for flux-shares' '
    flux account-shares -h > help_message.test
'

test_expect_success 'compare help message for flux-shares' '
    test_cmp ${EXPECTED_FILES}/help_message.expected help_message.test
'

test_expect_failure 'output help message for flux-shares when a bad argument is passed in' '
    flux account-shares --bad-arg > bad_argument.test
'

test_expect_success 'compare help message for flux-shares when a bad argument is passed in' '
    test_cmp ${EXPECTED_FILES}/help_message.expected bad_argument.test
'

test_expect_success 'create hierarchy output from C++ - out_of_insert_order.db' '
    flux account-shares -p ${OUT_OF_INSERT_ORDER} > out_of_insert_order.test
'

test_expect_success 'compare hierarchy outputs - out_of_insert_order.db' '
    test_cmp ${EXPECTED_FILES}/out_of_insert_order.expected out_of_insert_order.test
'

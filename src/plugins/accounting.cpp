                    ...
                    std::map<std::string, int> &priority_weights)
{
    ...
    if (json_unpack_ex (config_obj, &error, 0,
        ...
        if (db_associations) {
            if (load_associations (db_associations, users, users_def_bank) < 0)
                return -1;
        }
        if (db_queues) {
            if (load_queues (db_queues, queues) < 0)
                return -1;
        }
        if (db_projects) {
            if (load_projects (db_projects, projects) < 0)
                return -1;
        }
        if (db_banks) {
            if (load_banks (db_banks, banks) < 0)
                return -1;
        }
        if (db_factors) {
            if (load_priority_factors (db_factors, priority_weights) < 0)
                return -1;
        }
    }

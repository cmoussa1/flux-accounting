...
json_t* Association::to_json () const
{
    ...
    for (const auto &entry : queue_usage) {
        ...
        if (json_object_set_new (queue_usage_json, entry.first.c_str (), usage_object) < 0)
            goto error;
    }
...
bool Association::under_queue_max_run_jobs (
    ...
    bool under_queue_max_run_jobs = queue_usage[queue].cur_run_jobs < queues[queue].max_running_jobs;
...
bool Association::under_queue_max_resources (const Job &job,
                                             const Queue &queue)
{
    bool under_max_nodes = (queue_usage[queue.name].cur_nodes + job.nnodes) <= queue.max_nodes;

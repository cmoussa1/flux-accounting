...
void initialize_queues () {
    ...
    queues["bronze"].max_nodes = 1;
}


void queue_limits_defined ()
{
    ok (queues["bronze"].max_nodes == 1,
        "bronze queue has a max_nodes limit of 1");
}

/*
 * Without running any jobs, an association is under the queue's
 * max_nodes limit.
 */
 ...
 * per-queue dependency is added on any other submitted jobs until one of their
 * currently-running jobs has completed.
 */
void association_under_queue_max_nodes_limit_false ()
{
    ...
    // also to be submitted under the "bronze" queue
    Job job;
    job.id = 2;
    job.nnodes = 1;
    job.queue = "bronze";
    ...
    ok (a->under_queue_max_resources (job, queues["bronze"]) == false,
        "association is at or over queue's max_nodes limit");

    // since the association is at the queue's max_nodes limit, a dependency is
    // added to the second job to prevent it from running
    job.add_dep (D_QUEUE_MRES);
    a->held_jobs.emplace_back (job);

    ok (a->held_jobs.size () == 1,
        "association has one held job");
    ok (job.deps.size () == 1,
        "held job has one dependency added to it");
    ok (job.deps.front () == D_QUEUE_MRES,
        "held job contains a per-queue max resources dependency");
}

/*
 * Once the first job finishes running and current job (and resources) counters

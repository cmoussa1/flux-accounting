...
/*
 * Unpack a payload containing project information from an external bulk
 * update service and place it in the list of permissible projects that
 * user/banks can charge when submitting their job.
 */
                        ...
                        void *data)
{
    ...
    if (project != NULL) {
        if (validate_project (project, user_bank->projects) == INVALID_PROJECT)
            // the user/bank specified a project that they do not belong to or
            // that doesn't exist; reject the job
...
extern "C" int flux_plugin_init (flux_plugin_t *p)
{
    if (flux_plugin_register (p, "mf_priority", tab) < 0
        ...
        || flux_jobtap_service_register (p, "rec_prj_update", rec_p_cb, p) < 0)
        return -1;

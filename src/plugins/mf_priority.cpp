...
// Speculative "already released this pass" counters for a single held-job
// release sweep. Released jobs are not re-counted in an association's
// persistent counters until their own job.state.run / job.state.inactive
// callbacks fire, so without these a second held job would observe the same
// headroom as the first and be released even though the limit no longer
// permits it.
//
                        ...
                        void *data)
{
    ...
    if (!flux_jobtap_job_event_posted (p, FLUX_JOBTAP_CURRENT_JOB, "alloc")) {
        ...
        if (flux_jobtap_job_event_posted (p, FLUX_JOBTAP_CURRENT_JOB, "priority")) {
            ...
            // jobs and release them in jobid order.
            if (!b->held_jobs.empty ()) {
                std::vector<std::pair<Association *, Job *>> candidates;
                for (auto &held_job : b->held_jobs)
                    candidates.push_back (std::make_pair (b, &held_job));
    ...
    if (!b->held_jobs.empty ()) {
        ...
        // jobid order. The feature PR that adds a cross-association limit grows
        // this gather to span associations sharing the freed queue.
        std::vector<std::pair<Association *, Job *>> candidates;
        for (auto &held_job : b->held_jobs)
            candidates.push_back (std::make_pair (b, &held_job));
        if (release_held_jobs_ordered (p, candidates) < 0)
